...
class TestRelevanceDatasetBuilder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.train_file = tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False)
        self.civil_code_file = tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False)
        ...
        self.load_civil_code_patcher = patch('multidocqa.relevance_dataset.load_civil_code')
    ...
    def test_init(self):
        ...
        builder = RelevanceDatasetBuilder(
            ...
            self.civil_code_file.name,
            negative_ratio=3
        )
        
        self.assertEqual(builder.negative_ratio, 3)
        # Article lookup now includes paragraph variations (original + 3 variations each = 6 * 4 = 24)
    ...
    def test_generate_positive_pairs(self):
        ...
        article_statement_pairs = {
            (pair['article_number'], pair['statement_id']) 
            for pair in positive_pairs
        }
    ...
    def test_generate_negative_pairs(self, mock_choices, mock_sample):
        ...
        def choices_side_effect(population, k):
            return population[:k]
        
        mock_sample.side_effect = sample_side_effect
        mock_choices.side_effect = choices_side_effect
        
        negative_pairs = builder._generate_negative_pairs(2)  # 2 negatives per statement
    ...
    def test_negative_pairs_are_actually_negative(self):
    ...
    def test_build_relevance_dataset(self):
        ...
        # Should have positive pairs (5) + negative pairs (3 statements * 2 = 6) = 11 total
        ...
        required_keys = ['prompt', 'reference', 'article_number', 'statement', 'statement_id', 'pair_type']
    ...
    def test_get_dataset_statistics(self):
        ...
        self.assertEqual(stats['total_negative_pairs'], 9)  # 3 statements * 3 negatives per statement
...
class TestRelevanceUtilityFunctions(unittest.TestCase):
    ...
    def test_create_relevance_prompt_simple(self):
...
class TestRelevanceDatasetIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.comprehensive_train_data = [
            {
                ...
                "articles": [
                    ...
                    {
                        "number": "96", 
                        "content": "(1) A manifestation of intention based on fraud or duress is voidable. (2) If a third party commits a fraud inducing a first party to make a manifestation of intention to a second party, that manifestation of intention is voidable only if the second party knew or could have known that fact."
                    }
                ],
                "label": "N"
            },
            {
                "id": "H27-3-E",
                "question": "In the case where B, who was granted authority of agency to buy a land as an agent of A, was granted authority of agency to sell the land by C, became the agent of C, and concluded a contract for sale of a land 'X' between A and C, if B notified to both A and C in advance that B was the agent of both parties, the contract for sale binds A and C.",
                "articles": [
                    {
                        "number": "108",
                        "content": "(1) An act that a person performs as an agent of the counterparty or as agent of both parties for the same juridical act is deemed to be an act performed by a person without authority to represent; provided, however, that this does not apply to the performance of an obligation or to an act authorized by the principal in advance."
                    }
                ],
                "label": "N"
            }
        ...
        self.comprehensive_civil_code = [
            {"number": "96", "content": "(1) A manifestation of intention based on fraud or duress is voidable..."},
            {"number": "101", "content": "(1) If the validity of a manifestation of intention that an agent has made..."},
            {"number": "108", "content": "(1) An act that a person performs as an agent of the counterparty..."},
    ...
    def test_realistic_dataset_construction(self):
        """Test dataset construction with realistic data."""
        builder = RelevanceDatasetBuilder(
            "dummy_train.json",
            "dummy_civil_code.json", 
            negative_ratio=3
        )
        ...
        total_samples = len(train_dataset) + len(val_dataset)
        expected_total = 3 + (2 * 3)  # 3 positive + 6 negative (2 statements * 3 negatives per statement)
        ...
        negative_pairs = builder._generate_negative_pairs(100)  # Generate more than needed
        negative_combinations = {
            (pair['article_number'], pair['statement_id'])
            for pair in negative_pairs
        }
