...
class VllmEndpointGenerator:
    ...
    async def generate(self, texts: List[str], guided_json=None, **kwargs) -> List[str]:
        response_coroutines = []
        for text in texts:
            response = self._get_response(text, guided_json, **kwargs)
    ...
    async def _get_response(self, text: str, guided_json=None, **kwargs) -> str:
